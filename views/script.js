...
function onMouseMove(e) {
    if (mouseClicked) {
        ...
        let line = {"startX": prevX, "startY": prevY, "endX": client_X, "endY": client_Y, "color":color_details,"penSize":penSize};
        plots.push(line);
        peers.forEach(conn => {
...
async function downloadImage(e) {
    ...
    let filename = "board" + "-" + D[2] + "-" + D[1] + "-" + D[3] + "-" + D[4] + ".png";
...
async function downloadJSON(e) {
    ...
    document.getElementById('modal-wrapper').style.display='none';
    let D = Date().toString().split(" ");
    let filename = "board" + "-" + D[2] + "-" + D[1] + "-" + D[3] + "-" + D[4] + ".json";
    let dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(plots));
...
function uploadPlot() {
    ...
    el.addEventListener('change', function(ev2) {// access el.files[] to do something with it (test its length!)
        if (el.files.length) {
        ...
        new Promise(function(resolve) {
            setTimeout(function() {
                console.log(el.files[0].result); resolve();
